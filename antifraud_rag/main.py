...
class AntiFraudRAG:
    ...
    async def analyze(self, text: str) -> AnalysisResponse:
        ...
        try:
            ...
            # Chain with ``from e`` to preserve the original traceback.
            raise RuntimeError(f"Embedding error: {str(e)}") from e
        ...
        if not fused_results:
            ...
            return AnalysisResponse(
                ...
                data=RAGPromptData(
                    risk_level="LOW",
                    rrf_score=0.0,
                    # Use the shared prompt builder so the empty-match branch
                    # returns the same structured prompt as the medium branch
                    # (previously it returned just "分析用户请求: {text}").
                    prompt=build_rag_prompt(text, [], tips_data),
